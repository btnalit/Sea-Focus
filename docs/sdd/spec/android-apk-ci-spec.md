...
Sea Focus is a Vite React Android-targeted tomato planning app with a hand-drawn ocean/nature style. The delivery goal is to make the project capable of producing an Android release APK automatically from GitHub Actions.
...
- Add a GitHub Actions workflow that builds the web app, syncs Capacitor, assembles a release APK, and uploads the APK as a workflow artifact.
...
- Play Store upload and AAB publishing.
...
React/Vite/Tailwind app -> static `dist` output -> Capacitor web view -> Android Gradle project -> release APK.
...
- Release APK artifact source: `android/app/build/outputs/apk/release/*.apk`
- GitHub artifact name: `sea-focus-release-apk`
...
- Android Gradle `assembleRelease` validates native APK generation.
